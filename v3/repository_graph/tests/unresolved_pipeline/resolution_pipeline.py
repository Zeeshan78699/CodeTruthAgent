...
from .flatten_class_graph import flatten_class_graph, flatten_return_type_table
...
def _looks_like_factory_name(function_name, flat_class_graph):
    if not function_name or function_name in flat_class_graph:
        return False
    return any(function_name.startswith(p) for p in FACTORY_PREFIXES)


def _enrich_origin_facts(extracted_facts, assignment_table, flat_class_graph, origin_extractor):
    """
        ...
        if fact.get("pattern") == "attribute_call" and fact.get("attribute_name"):
            ...
            if variable_name:
                ...
                if origin and origin.get("origin_type") == "call":
                    ...
                    if candidate in flat_class_graph:
                        fact["constructor_class"] = candidate
                    elif _looks_like_factory_name(candidate, flat_class_graph):
...
class ResolutionPipeline:
    ...
    def __init__(self, unresolved_entries, return_type_table, class_graph,
        ...
        self.assignment_table = build_assignment_table(repo_path) if repo_path else {}
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if self.origin_extractor is not None:
            extracted_facts = _enrich_origin_facts(
                ...
                self.flat_class_graph, self.origin_extractor,
            )
        ...
        factory_results = run_factory_return_engine(
            remaining, self.flat_return_type_table, self.flat_class_graph
        )
        remaining = factory_results["remaining_entries"]

        if self.repo_path:
            property_type_table = build_property_type_table(self.repo_path, self.flat_class_graph)
        else:
            property_type_table = {}
        ...
        # FIX: use the flattened class graph.
        inheritance_resolver = InheritanceResolver(self.flat_class_graph)
