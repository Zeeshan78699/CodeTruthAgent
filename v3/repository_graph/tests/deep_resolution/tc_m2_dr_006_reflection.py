"""TC_M2_DR_006 — Deep Resolution: Reflection Resolver (Known Gap)"""
import json, sys
...
TEST_ID      = "TC_M2_DR_006"
EVIDENCE_DIR = Path(__file__).parent / "evidence"
FIXTURE_DIR  = Path(__file__).parent / "fixtures" / "reflection"
...
def save_markdown(passed, result, note=""):
    ...
    lines = [
        ...
        f"| dr_reflection | {result.get('dr_reflection', 'N/A')} |",
    ...
    lines += [
        ...
        "", "| Requirement | Status |", "|---|---|",
        f"| DR-006 Reflection Resolver (Known Gap) | {status} |",
    ]
    md.write_text("\n".join(lines), encoding="utf-8")
    print(f"Evidence saved --> {md}")

def test_tc_m2_dr_006():
    print("=" * 80)
    print("TC_M2_DR_006 — Deep Resolution: Reflection Resolver (Known Gap)")
    print("=" * 80)
    create_fixture()
    ...
    print(f"     dr_reflection           : {result['dr_reflection']}")
    print(f"     dr_resolved_by_pipeline : {result['dr_resolved_by_pipeline']}")
    print(f"     dr_reduction_pct        : {result['dr_reduction_pct']}")

    print("\nASSERTION\n" + "-" * 60)
    val = result["dr_reflection"]
    print(f"PASS  dr_reflection = {val} (0 expected — documented gap)")
    note = f"dr_reflection={val} — dynamic getattr() not statically resolvable. Documented known gap."
