...
========================================================================
TEST ID:        TC_M1_015
TITLE:          Industrial Control System Repository Validation
GROUP:          B
MODULE:         Module 1 - Repository Cognition Engine
VERSION:        2.0

OBJECTIVE:
    Validate Industrial Control-domain repository cognition.

REPOSITORY:
    pymodbus — Modbus protocol for industrial PLCs and SCADA
...
TEST_ID   = "TC_M1_015"
REPO_PATH = r"C:\repos\v3\pymodbus"
ACCEPTABLE_APPLICATION_TYPES = {"MANUFACTURING_SYSTEM", "ENERGY_SYSTEM", "INDUSTRIAL_SYSTEM", "API_SERVICE", "EMBEDDED_SYSTEM"}
...
def save_markdown(core, enhanced, passed):
    ...
    lines = [
        f"# {TEST_ID} — Industrial Control System Repository Validation",
        "", "| Field | Value |", "|---|---|",
        f"| Status | {status} |",
        f"| Execution Date | {dt.now(UTC).date().isoformat()} |",
        f"| Repository | pymodbus |",
        "", "## Core",
        "", "| Field | Value |", "|---|---|",
...
def test_tc_m1_015_pymodbus_repository():
    print("=" * 80)
    print(f"{TEST_ID} — Industrial Control System Repository Validation")
    print("=" * 80)
...
if __name__ == "__main__":
    try:
        test_tc_m1_015_pymodbus_repository()
