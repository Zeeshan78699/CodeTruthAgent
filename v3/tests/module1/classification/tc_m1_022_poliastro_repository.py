...
========================================================================
TEST ID:        TC_M1_023
TITLE:          Planetary Science Repository Validation
GROUP:          D
MODULE:         Module 1 - Repository Cognition Engine
VERSION:        2.0

OBJECTIVE:
    Validate Planetary Science-domain repository cognition.

REPOSITORY:
    astropy — Core astronomy and planetary science tools
...
TEST_ID   = "TC_M1_023"
REPO_PATH = r"C:\repos\v3\astropy"
# Open the file and change line 57 to:
ACCEPTABLE_APPLICATION_TYPES = {"SCIENTIFIC_SYSTEM", "AEROSPACE_SYSTEM", "CLIMATE_SCIENCE", "SPACE_SYSTEM", "UNKNOWN", "SIMULATION_TOOL", "DATA_ENGINEERING"}
...
def save_markdown(core, enhanced, passed):
    ...
    lines = [
        f"# {TEST_ID} — Planetary Science Repository Validation",
        "", "| Field | Value |", "|---|---|",
        f"| Status | {status} |",
        f"| Execution Date | {dt.now(UTC).date().isoformat()} |",
        f"| Repository | astropy |",
        "", "## Core",
        "", "| Field | Value |", "|---|---|",
...
def test_tc_m1_023_astropy_repository():
    print("=" * 80)
    print(f"{TEST_ID} — Planetary Science Repository Validation")
    print("=" * 80)
    ...
    # signal analyzer has no matching generic domain keywords.
    # Core classification is the authority. NONE here is not a failure.
...
if __name__ == "__main__":
    try:
        test_tc_m1_023_astropy_repository()
