...
========================================================================
TEST ID:        TC_M1_013
TITLE:          Banking Repository Validation
GROUP:          B
MODULE:         Module 1 - Repository Cognition Engine
VERSION:        2.0

OBJECTIVE:
    Validate Banking-domain repository cognition.

REPOSITORY:
    zipline — Quantitative trading backtesting platform
...
TEST_ID   = "TC_M1_013"
REPO_PATH = r"C:\repos\v3\zipline"
ACCEPTABLE_APPLICATION_TYPES = {"FINANCE_SYSTEM", "BANKING_SYSTEM", "ML_PIPELINE", "AI_ML_SYSTEM"}
...
def save_markdown(core, enhanced, passed):
    ...
    lines = [
        f"# {TEST_ID} — Banking Repository Validation",
        "", "| Field | Value |", "|---|---|",
        f"| Status | {status} |",
        f"| Execution Date | {dt.now(UTC).date().isoformat()} |",
        f"| Repository | zipline |",
        "", "## Core",
        "", "| Field | Value |", "|---|---|",
...
def test_tc_m1_013_zipline_repository():
    print("=" * 80)
    print(f"{TEST_ID} — Banking Repository Validation")
    print("=" * 80)
...
if __name__ == "__main__":
    try:
        test_tc_m1_013_zipline_repository()
