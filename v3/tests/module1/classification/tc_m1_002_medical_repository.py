...
========================================================================
TEST ID:        TC_M1_002
TITLE:          Medical Repository Validation
MODULE:         Module 1 - Repository Cognition Engine
VERSION:        2.0

OBJECTIVE:
    Validate Medical-domain repository cognition against a real
    open-source repository.

REPOSITORY:
    pydicom — confirmed 69-repo corpus member

EXPECTED:
    Application Type   = MEDICAL_SYSTEM (or acceptable variant)
...
TEST_ID   = "TC_M1_002"
REPO_PATH = r"C:\repos\v3\pydicom"
ACCEPTABLE_APPLICATION_TYPES = {"MEDICAL_SYSTEM"}
...
def save_markdown(core, enhanced, passed):
    ...
    lines = [
        f"# {TEST_ID} — Medical Repository Validation",
        "", f"| Field | Value |", f"|---|---|",
        f"| Status | {status} |",
        f"| Execution Date | {dt.now(UTC).date().isoformat()} |",
        f"| Repository | pydicom |",
        "", "## Core",
        "", f"| Field | Value |", f"|---|---|",
...
def test_tc_m1_002_pydicom_repository():
    print("=" * 80)
    print(f"{TEST_ID} — Medical Repository Validation")
    print("=" * 80)
...
if __name__ == "__main__":
    try:
        test_tc_m1_002_pydicom_repository()
