...
========================================================================
TEST ID:        TC_M1_008
TITLE:          ML Pipeline Repository Validation
MODULE:         Module 1 - Repository Cognition Engine
VERSION:        2.0

OBJECTIVE:
    Validate ML-domain repository cognition against a real
    open-source repository.

REPOSITORY:
    transformers — confirmed 69-repo corpus member

EXPECTED:
    Application Type   = ML_SYSTEM (or acceptable variant)
...
TEST_ID   = "TC_M1_008"
REPO_PATH = r"C:\repos\v3\transformers"
ACCEPTABLE_APPLICATION_TYPES = {"AI_ML_SYSTEM", "ML_SYSTEM", "NLP_SYSTEM", "ML_PIPELINE"}
...
def save_markdown(core, enhanced, passed):
    ...
    lines = [
        f"# {TEST_ID} — ML Pipeline Repository Validation",
        "", f"| Field | Value |", f"|---|---|",
        f"| Status | {status} |",
        f"| Execution Date | {dt.now(UTC).date().isoformat()} |",
        f"| Repository | transformers |",
        "", "## Core",
        "", f"| Field | Value |", f"|---|---|",
...
def test_tc_m1_008_transformers_repository():
    print("=" * 80)
    print(f"{TEST_ID} — ML Pipeline Repository Validation")
    print("=" * 80)
...
if __name__ == "__main__":
    try:
        test_tc_m1_008_transformers_repository()
