"""
factory_origin_tracker.py

Purpose:
Identify variables originating from factory functions.
...
Truth Boundary:
- Factory must be explicitly identified
- Constructor calls excluded
- No guessing
"""
...
class FactoryOriginTracker:
    ...
    def build(self):
        ...
        for variable_key, origin in (
            ...
            if not self._looks_like_factory(
                candidate
            ):
                continue
...
def build_factory_origins(
    assignment_table,
    class_graph
):

    tracker = FactoryOriginTracker(
        assignment_table,
        class_graph
    )
