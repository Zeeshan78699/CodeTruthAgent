...
========================================================================
TEST ID:        TC_M1_006
TITLE:          API Service Repository Validation
MODULE:         Module 1 - Repository Cognition Engine
VERSION:        2.0

OBJECTIVE:
    Validate API-domain repository cognition against a real
    open-source repository.

REPOSITORY:
    fastapi — confirmed 69-repo corpus member

EXPECTED:
    Application Type   = API_SERVICE (or acceptable variant)
...
TEST_ID   = "TC_M1_006"
REPO_PATH = r"C:\repos\v3\fastapi"
ACCEPTABLE_APPLICATION_TYPES = {"API_SERVICE", "WEB_APPLICATION"}
...
def save_markdown(core, enhanced, passed):
    ...
    lines = [
        f"# {TEST_ID} — API Service Repository Validation",
        "", f"| Field | Value |", f"|---|---|",
        f"| Status | {status} |",
        f"| Execution Date | {dt.now(UTC).date().isoformat()} |",
        f"| Repository | fastapi |",
        "", "## Core",
        "", f"| Field | Value |", f"|---|---|",
...
def test_tc_m1_006_fastapi_repository():
    print("=" * 80)
    print(f"{TEST_ID} — API Service Repository Validation")
    print("=" * 80)
...
if __name__ == "__main__":
    try:
        test_tc_m1_006_fastapi_repository()
