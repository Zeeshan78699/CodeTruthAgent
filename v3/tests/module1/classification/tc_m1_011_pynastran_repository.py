...
========================================================================
TEST ID:        TC_M1_011
TITLE:          Aerospace Repository Validation
GROUP:          B
MODULE:         Module 1 - Repository Cognition Engine
VERSION:        2.0

OBJECTIVE:
    Validate Aerospace-domain repository cognition.

REPOSITORY:
    pyNastran — Finite element analysis for aerospace structures
...
TEST_ID   = "TC_M1_011"
REPO_PATH = r"C:\repos\v3\pyNastran"
ACCEPTABLE_APPLICATION_TYPES = {"AEROSPACE_SYSTEM", "SCIENTIFIC_SYSTEM", "AEROSPACE", "SIMULATION_TOOL"}
...
def save_markdown(core, enhanced, passed):
    ...
    lines = [
        f"# {TEST_ID} — Aerospace Repository Validation",
        "", "| Field | Value |", "|---|---|",
        f"| Status | {status} |",
        f"| Execution Date | {dt.now(UTC).date().isoformat()} |",
        f"| Repository | pyNastran |",
        "", "## Core",
        "", "| Field | Value |", "|---|---|",
...
def test_tc_m1_011_pynastran_repository():
    print("=" * 80)
    print(f"{TEST_ID} — Aerospace Repository Validation")
    print("=" * 80)
    ...
    # Evidence strength may be NONE for highly specialized repos
    # where signal analyzer has no matching domain keywords.
    # Core classification is the authority — if application_type
    # is valid, the classification succeeded regardless of signal strength.
    ...
    print(f"     Strength   : {enhanced.classification_reason.evidence_strength}"
          + (" [Core classification authoritative]"
             if enhanced.classification_reason.evidence_strength == "NONE" else ""))
...
if __name__ == "__main__":
    try:
        test_tc_m1_011_pynastran_repository()
